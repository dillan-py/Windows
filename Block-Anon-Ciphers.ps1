...
<#
.SYNOPSIS
    Configures TLS on a Windows print server to disable TLS 1.0/1.1 and anonymous ciphers, enable TLS 1.2/1.3, and manage print services.

.DESCRIPTION
    This script:
    - Backs up the SCHANNEL registry.
    - Disables TLS 1.0/1.1 and anonymous ciphers (AECDH-*).
    - Enables TLS 1.2/1.3 and secure, authenticated ciphers.
    ...
    - Logs actions to C:\Logs\TLS_Configuration_Log.txt.

.NOTES
    - Must be run as Administrator.
    - Test in a non-production environment first.
    - Disables anonymous ciphers to address SSL Anonymous Cipher Suites vulnerability.
...
$AnonymousCiphers = @("AECDH-AES128-SHA", "AECDH-AES256-SHA")
try {
    foreach ($Cipher in $AnonymousCiphers) {
        $CipherPath = "$SchannelCiphersPath\$Cipher"
        if (-not (Test-Path $CipherPath)) {
            New-Item -Path $CipherPath -Force | Out-Null
            Write-Log "Created registry path for cipher: ${CipherPath}"
        }
        Set-ItemProperty -Path $CipherPath -Name "Enabled" -Value 0 -Type DWORD -Force
        Write-Log "Disabled anonymous cipher: $Cipher"
    }
} catch {
    Write-Log "ERROR: Failed to disable anonymous ciphers: $_"
    Write-Warning "Failed to disable anonymous ciphers. Manual configuration may be required."
}
